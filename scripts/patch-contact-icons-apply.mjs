...
function patchVite(){
  ...
  // WhatsApp anchor
  s = s.replace(
    /<li><a href=\{(`https:\/\/wa\.me\/\$\{[^}]+\}`)\} target="_blank" rel="noopener noreferrer" className="underline hover:text-white">WhatsApp<\/a><\/li>/,
    '<li><a href={$1} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 underline hover:text-white"><FaWhatsapp className="text-green-500" /> WhatsApp</a></li>'
  )
  // Email anchor
  s = s.replace(
    /<li><a className="underline hover:text-white" href=\{(`mailto:\$\{emailSales\}`)\}>\{lang === 'CN' \? '[^']*' : 'Email'\}<\/a><\/li>/,
    '<li><a className="flex items-center gap-2 underline hover:text-white" href={$1}><FiMail className="text-white/80" /> {lang === \"CN\" ? \"邮件\" : \"Email\"}</a></li>'
  )
  // Resources link
  s = s.replace(
...
function patchNext(){
  ...
  // Live Chat list: replace the first UL under Live Chat heading
  s = s.replace(
    /(<h3 className=\"text-lg font-semibold text-white\">\{locale === 'CN' \? '[^']*' : 'Live Chat & Social'\}<\/h3>\s*<ul className=\"mt-2 space-y-2 text-sm text-white\/75\">)[\s\S]*?<\/ul>/,
  ...
  // Fallbacks list: replace the next UL under Fallbacks heading
  s = s.replace(
    /(<h3 className=\"text-lg font-semibold text-white\">\{locale === 'CN' \? '[^']*' : 'Fallbacks'\}<\/h3>\s*<ul className=\"mt-2 space-y-2 text-sm text-white\/75\">)[\s\S]*?<\/ul>/,
