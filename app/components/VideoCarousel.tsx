...
function normalise(value: unknown): VideoItem[]{
  if (!value) return []
  if (Array.isArray(value)){
    return value.map((entry) => {
      if (typeof entry === 'string') return { title: entry }
      if (entry && typeof entry === 'object'){
        const { title, description, desc, src, thumbnail, script_en, script_cn } = entry as Record<string, unknown>
        return {
...
export default function VideoCarousel({ dict, path, items, locale = 'EN' as Locale }: VideoCarouselProps){
  const derived = useMemo(() => {
    if (items?.length) return items
    if (dict && path){
      const value = pick(dict, path)
      const list = normalise(value)
          ...
          <div className="relative z-10 w-[min(880px,92vw)] max-h-[86vh] overflow-auto rounded-2xl border border-white/15 bg-black/90 p-5 shadow-2xl">
            ...
            <div className="mt-3 space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div className="inline-flex overflow-hidden rounded-lg border border-white/15 text-xs">
                  <button
                    className={`px-3 py-1.5 ${modalTab === 'video' ? 'bg-white/15 text-white' : 'text-white/70 hover:bg-white/10'}`}
                    onClick={() => setModalTab('video')}
                  >{lang === 'CN' ? '视频' : 'Video'}</button>
                  <button
                    className={`px-3 py-1.5 ${modalTab === 'script' ? 'bg-white/15 text-white' : 'text-white/70 hover:bg-white/10'}`}
                    onClick={() => setModalTab('script')}
                  >{lang === 'CN' ? '脚本' : 'Script'}</button>
                </div>
                <div className="inline-flex overflow-hidden rounded-lg border border-white/15 text-xs">
                  <button
                    className={`px-3 py-1.5 ${lang === 'EN' ? 'bg-white/15 text-white' : 'text-white/70 hover:bg-white/10'}`}
              ...
              {modalTab === 'video' ? (
                derived[openIndex!]?.src ? (
                  ...
                  <div className="rounded-xl border border-white/15 bg-black/40 p-4 text-sm text-white/70">
                    {lang === 'CN' ? '暂无视频源，请稍后添加。' : 'No video source yet. Add a src to enable playback.'}
                  </div>
                )
              ) : (
                <p className="whitespace-pre-wrap text-sm leading-relaxed text-white/80">
                  {lang === 'EN' ? (derived[openIndex!]?.script_en || 'Script not available.') : (derived[openIndex!]?.script_cn || '暂无脚本。')}
                </p>
              )}
            </div>
          </div>
