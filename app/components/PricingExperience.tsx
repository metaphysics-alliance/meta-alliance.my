...
export type PricingDictionary = {
  ...
  checkoutAssuranceTitle?: string
  checkoutAssurancePoints?: string[]
}
...
function ExperienceContent({ locale, pricing }: { locale: Locale; pricing: PricingDictionary }) {
  ...
  const checkoutHref = `/${locale}/pricing/checkout`
  const exploreLabel = locale === 'CN' ? '浏览方案' : 'Browse programmes'
  const strategistLabel = locale === 'CN' ? '与顾问沟通' : 'Talk to a strategist'

  return (
    <div className="space-y-20 pb-24 text-white" id="pricing-top">
      <HeroBlock
        locale={locale}
      ...
      <section id="catalog" className="container space-y-10">
        <SectionDivider
      ...
      {addOns.length ? (
        <section className="container space-y-8">
          <SectionDivider
            title={locale === 'CN' ? '增值组件' : 'Retainers & amplifiers'}
      ...
      {objections.length ? (
        <section className="container space-y-8">
          <SectionDivider
            title={pricing.objectionsTitle ?? (locale === 'CN' ? '我们消除前五大拒绝理由' : 'Top five hesitations, disarmed')}
            subtitle={
              pricing.objectionsSubtitle ?? (locale === 'CN' ? '常见的「不要」，一次性回应' : 'Every typical “no” now has a countermeasure')
            }
          />
...
function NoticeGrid({ points, currencyNote }: { points: string[]; currencyNote?: string }) {
...
function CategoryPanel({ locale, category, cartLabels }: { locale: Locale; category: PricingCategory; cartLabels: CartLabels }) {
...
function TierCard({
  ...
  const internalHref = tier.href && !isExternal ? `/${locale}${tier.href}` : tier.href

  const cartEntry = useMemo<CartEntry>(
    () => ({
    ...
    [category, tier, internalHref, locale],
  )
      ...
      <div className="flex flex-col gap-4">
        <header className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap items-baseline gap-3 text-sm text-white/70">
            ...
            {tier.cadence ? <span className="rounded-full border border-white/20 px-3 py-1 text-xs uppercase tracking-[0.22em] text-white/70">{tier.cadence}</span> : null}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 py-6">
          ...
          {tier.href ? (
            isExternal ? (
              ...
              <Link className="inline-flex items-center gap-2 text-sm font-medium text-gold hover:text-white" href={internalHref ?? '#'}>
                {locale === 'CN' ? '查看详情' : 'View details'}
                <FiArrowRight className="h-4 w-4" />
              </Link>
            )
...
function AddonCard({ locale, addOn, cartLabels }: { locale: Locale; addOn: PricingAddOn; cartLabels: CartLabels }) {
  const { toggleItem, isInCart } = usePricingCart()
...
function ObjectionCard({
  ...
  entry: PricingDictionary['objections'][number]
  locale: Locale
}) {
...
function CartDock({ locale, labels, checkoutHref }: { locale: Locale; labels: CartLabels; checkoutHref: string }) {
...
function buildCartId(category: PricingCategory, tier: PricingTier) {
...
export function resolveCartLabels(cart: PricingDictionary['cart'], locale: Locale): CartLabels {
  return {
