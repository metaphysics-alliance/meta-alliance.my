...
export function generateMetadata({ params }: { params: { locale: Locale } }): Metadata {
  ...
  const title = dictTitle(dict as any, 'nav.fengshui') || 'Feng Shui Consultation - Home (Small)'
  const desc = locale === 'CN' ? '小户型住宅风水咨询与布局。' : 'Small residential Feng Shui consultation and layout.'
  return { title, description: desc, alternates: { canonical: `/${locale}/services/fengshui/consultation/home-small`, languages: { en: '/EN/services/fengshui/consultation/home-small', zh: '/CN/services/fengshui/consultation/home-small' } }, openGraph: { title, description: desc, url: `/${locale}/services/fengshui/consultation/home-small` } }
}

export default function Page({ params }:{ params:{ locale: Locale }}){
  const dict = getDict(params.locale);
  const title = dictTitle(dict, 'nav.fengshui') || 'Feng Shui Consultation — Home (Small)'
  const base = process.env.NEXT_PUBLIC_SITE_URL || ''
  return (
    <div className='space-y-10'>
      <StructuredData json={{ '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [ { '@type': 'ListItem', position: 1, name: params.locale, item: `${base}/${params.locale}` }, { '@type': 'ListItem', position: 2, name: 'Services', item: `${base}/${params.locale}/services` }, { '@type': 'ListItem', position: 3, name: String(title), item: `${base}/${params.locale}/services/fengshui/consultation/home-small` } ] }} />
