...
export function generateMetadata({ params }: { params: { locale: Locale } }): Metadata {
  ...
  const title = dictTitle(dict as any, 'svc.qimen')
  const desc = (dict as any).why_long || 'Qi Men Dun Jia strategy aligned to practical timing windows.'
  return {
    title: String(title),
    ...
    openGraph: { title: String(title), description: String(desc).slice(0, 160), url: `/${locale}/services/qimen` },
  }
}

export default function Page({ params }:{ params:{ locale: Locale }}){
  const dict = getDict(params.locale);
  const title = dictTitle(dict, 'svc.qimen');
    ...
    <div className='space-y-10'>
      <StructuredData json={{
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: String(title), item: `${base}/${params.locale}/services/qimen` },
        ]
      }} />
      <StructuredData json={{
        ...
        url: `${base}/${params.locale}/services/qimen`
      }} />
      <Hero title={title} />
      <Content title={title} />
      <MediaGrid/>
